...
-- Reference for SOFA:
--    Jean-Louis Vincent, Rui Moreno, Jukka Takala, Sheila Willatts, Arnaldo De Mendonca,
...
(
  ...
  , (ih.endtime - INTERVAL '1 hours') AS starttime
  , ih.endtime
  from {mimic_derived}.icustay_hours ih
...
(
  ...
  inner join {mimic_derived}.pivoted_bg_art bg
...
(
  ...
  left join {mimic_derived}.pivoted_gcs gcs
    on co.icustay_id = gcs.icustay_id
    and co.starttime < gcs.charttime
    and co.endtime >= gcs.charttime
  left join {mimic_derived}.pivoted_lab labs
  ...
  left join {mimic_derived}.pivoted_bg_art bg
...
(
  ...
  from co
  left join {mimic_derived}.pivoted_uo uo
...
(
  ...
  left join {mimic_derived}.epinephrine_dose epi
    on co.icustay_id = epi.icustay_id
    and co.endtime > epi.starttime
    and co.endtime <= epi.endtime
  left join {mimic_derived}.norepinephrine_dose nor
    on co.icustay_id = nor.icustay_id
    and co.endtime > nor.starttime
    and co.endtime <= nor.endtime
  left join {mimic_derived}.dopamine_dose dop
    on co.icustay_id = dop.icustay_id
    and co.endtime > dop.starttime
    and co.endtime <= dop.endtime
  left join {mimic_derived}.dobutamine_dose dob

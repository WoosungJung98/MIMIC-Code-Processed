...
(
  select
    ...
    , Extubated
    , SelfExtubated
  from {mimic_derived}.ventilation_classification
)
, vd1 as
(
  select
      ...
      , case
          ...
          when MechVent=1 then
            cast(extract(epoch from CHARTTIME - charttime_lag)/3600.0 as int)
          else null
        end as ventduration
      , LAG(Extubated,1)
      OVER
...
insert into {mimic_derived}.ventilation_durations
...
select icustay_id
  ...
  , cast(extract(epoch from max(charttime) - min(charttime))/3600.0 as int) AS duration_hours
from vd2

...
( -- begin query that extracts the data
  ...
  -- this also fuses together multiple ITEMIDs containing the same data
    ...
    ELSE valuenum END)::numeric, 2) as double precision) AS valuenum
...
(
  select hadm_id, charttime
    ...
    )::numeric, 2) as double precision) as fio2_chartevents
  FROM {mimic_clinical}.chartevents
  where ITEMID in
  (
    3420 -- FiO2
  , 190 -- FiO2 set
...
insert into {mimic_derived}.pivoted_fio2
select
  ie.icustay_id
...
INNER JOIN {mimic_clinical}.icustays ie
  on base.hadm_id = ie.hadm_id
  AND base.charttime >= (ie.intime - INTERVAL '12 hours')
  AND base.charttime <= (ie.outtime + INTERVAL '12 hours')
LEFT JOIN pvt

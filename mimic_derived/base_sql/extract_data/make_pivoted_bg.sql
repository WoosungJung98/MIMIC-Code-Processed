...
(
  select
    ...
    , case
        ...
        and i.outtime_lag > (i.intime - INTERVAL '24 hours')
          then (i.intime - make_interval(hours => cast(round(extract(epoch from i.intime - i.outtime_lag)/7200.0) as int)))
      else (i.intime - INTERVAL '12 hours')
      end as data_start
    , case
        when i.intime_lead is not null
        and i.intime_lead < (i.outtime + INTERVAL '24 hours')
          then (i.outtime + make_interval(mins => cast(round(extract(epoch from i.intime_lead - i.outtime)/120.0) as int)))
      else (i.outtime + INTERVAL '12 hours')
      end as data_end
    from i
)
, pvt as
( -- begin query that extracts the data
  ...
  -- this also fuses together multiple ITEMIDs containing the same data
    ...
    FROM {mimic_clinical}.labevents le
    where le.ITEMID in
    -- blood gases
    (
...
insert into {mimic_derived}.pivoted_bg
select
  iid.icustay_id, grp.*
from grp
inner join {mimic_clinical}.admissions adm

...
(
    ...
    FROM {mimic_clinical}.chartevents c
...
(
  ...
  inner join {mimic_derived}.echo_data ec
...
(
  select
    ...
    , coalesce(
        ...
        (GREATEST(outtime, starttime) + INTERVAL '2 hours')
      ) as endtime
    , weight
  from wt_stg2
)
...
insert into {mimic_derived}.weight_durations
select
    wt1.icustay_id
  , wt1.starttime
  , wt1.endtime

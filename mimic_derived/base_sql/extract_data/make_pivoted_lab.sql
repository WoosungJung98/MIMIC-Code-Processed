  ...
  select
    ...
    , case
        ...
        and i.outtime_lag > (i.intime - INTERVAL '24 hours')
          then (i.intime - make_interval(secs => cast(extract(epoch from i.intime - i.outtime_lag)/2 as bigint)))
      else (i.intime - INTERVAL '12 hours')
      end as data_start
    , case
        when i.intime_lead is not null
        and i.intime_lead < (i.outtime + INTERVAL '24 hours')
          then (i.outtime + make_interval(secs => cast(extract(epoch from i.intime_lead - i.outtime)/2 as bigint)))
...
insert into {mimic_derived}.pivoted_lab
select
